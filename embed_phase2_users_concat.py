...
train_tensor = tokenizer(train_df['Long_Answer_concat'].to_list(), max_length=model.config.max_position_embeddings, return_tensors='pt', padding=True)
test_tensor = tokenizer(test_df['Long_Answer_concat'].to_list(), max_length=model.config.max_position_embeddings, return_tensors='pt', padding=True)
...
torch.save(train_result, f'{model_name.replace("/", "_")}_train_phase2_merged_sorted_{k_user_ids}users_concat.pt')
torch.save(test_result, f'{model_name.replace("/", "_")}_test_phase2_merged_sorted_{k_user_ids}users_concat.pt')
